...
const Deposit = (prop) => {
                    ...
                    <div>
                      ...
                      {prop.permission.approve_deposite == 1 ? (
                        <MenuItem
                          ...
                          {...row}
                          onClick={(event) =>
                            actionMenuPopup(event, row.deposit_id, "approve")
                          }
                        >
                      ...
                      {prop.permission.reject_deposite == 1 ? (
                        <MenuItem
                          ...
                          {...row}
                          onClick={(event) =>
                            actionMenuPopup(event, row.deposit_id, "reject")
                          }
                        >
  ...
  const manageDialogActionButton = () => {
    ...
    } else if (dialogTitle == "Deposit Proof") {
      return (
        <div className="dialogMultipleActionButton">
          <Button
            variant="contained"
  ...
  const submitUpdate = async () => {
    if (viewDepositForm.amount == "") {
      ...
      (viewDepositForm.deposit_method == "USDT.TRC20" ||
        viewDepositForm.deposit_method == "BTC" ||
        viewDepositForm.deposit_method == "ETH" ||
        viewDepositForm.deposit_method == "LTC") &&
      viewDepositForm.crypto_address == ""
    ) {
      ...
      param.append("admin_notes", viewDepositForm.admin_notes);
      if (
        viewDepositForm.deposit_method == "USDT.TRC20" ||
        viewDepositForm.deposit_method == "BTC" ||
        viewDepositForm.deposit_method == "ETH" ||
        ...
        param.append("crypto_address", viewDepositForm.crypto_address);
      }
          ...
          <div className="update-withdraw-request-section">
            ...
            viewDepositForm.deposit_method == "LTC" ? (
              <TextField
                ...
                sx={{ width: "100%" }}
                name="crypto_address"
                value={viewDepositForm.crypto_address}
                onChange={input1}
                focused
                disabled={
                  storeDepositData.status == "0" &&
  ...
  const viewDeposit = async (data) => {
    handleContextClose(data.deposit_id);
